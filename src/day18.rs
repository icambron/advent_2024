...
impl Solver for Day18 {
    ...
    fn parse(&self, input: &str, is_sample: bool) -> Self::Input {
       ...
       let drops: Vec<usize> = input.lines().map(|line| {
        ...
        Memory { drops, dimension, how_many }
    }
...
impl Memory {
    fn next_coord(&self, old_coord: usize) -> [Option<usize>; 4]{
        [
            if old_coord % self.dimension > 0 { Some(old_coord - 1) } else { None },
            if old_coord % self.dimension < self.dimension - 1 { Some(old_coord + 1) } else { None },
            if old_coord >= self.dimension { Some(old_coord - self.dimension) } else { None },
            if old_coord < self.dimension * (self.dimension - 1) { Some(old_coord + self.dimension) } else { None }
        ]
    }
    ...
    fn find_path_breadth_first(&self, space: &mut [bool]) -> Option<usize> {
        ...
        while let Some((coord, steps)) = queue.pop_back() {
            ...
            *s = true;
            ...
            if coord == self.dimension * self.dimension - 1 {
                return Some(steps)
            }
